...
function App() {
  ...
  const PatientDashboard = ({ handleLogout, setCurrentScreen, setReviewDoctorId, appointments, upcomingAppointment, userName, setCallPartner }) => {
    ...
    <button 
        onClick={() => { setReviewDoctorId(apt.doctor_id); setCurrentScreen('rate-doctor'); }} 
        className="mt-1 inline-flex items-center gap-1 bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-xs font-bold hover:bg-yellow-200 transition-colors shadow-sm"
    >
        ⭐ Rate
    </button>
) : (
    <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${
        ...
        (apt.status === 'upcoming' ? 'bg-teal-100 text-teal-700' : 'bg-gray-200 text-gray-700'))
    }`}>
        {apt.status}
    </span>
)}
